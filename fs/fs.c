...
static void partition_format(struct partition *part){
	...
	uint32_t inode_secs = DIV_ROUND_UP(sizeof(inode) * MAX_FILES_PER_PART,
		       	SECTOR_SIZE);
	//先统计出字节数, 再统计扇区数
...
//读取超级块
	...
	ide_read(part->hd, part->start_lba + 1, part->sb, 1);
...
//读取block位图
	...
	ide_read(part->hd, sb->block_bitmap_lba, bmp.bits, sb->block_bitmap_sects);
...
//读取inode 位图
	...
	ide_read(part->hd, sb->inode_bitmap_lba, bmp.bits, sb->inode_bitmap_sects);
}
