...
static void busy_wait(struct disk *hd){
	...
	while(1){
		...
		//硬盘不忙
		if(!(inb(reg_status(hd->channel)) & BIT_STAT_BSY)){
...
static inline void swap_copy(const char *src, char *buf, uint32_t size){
...
static void identify_disk(struct disk *hd){
	...
	char buf[3][64];
	swap_copy(info + 10 * 2, buf[0], 10);
	swap_copy(info + 27 * 2, buf[1], 40);
	swap_copy(info + 60 * 2, buf[2], 2);
...
void ide_init(){
	...
	while(channel_no < channel_cnt){
		...
		//注册中断处理程序
		register_callback(channels[channel_no].irq_no, intr_hd_handler);
		++channel_no;
	}
